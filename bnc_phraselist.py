...
phrase=[i for i in phrases_list if len(i.split())>1]
print('phrase:',len(phrase))


os.chdir('C:/')
''' get the text from corpus, since spoken bnc corpus compiled in xml format, i used xml reader code. ('s'==sentence, 'w'==word) '''
sentences=[]
for file_root, dirs, files in os.walk('BNC'):
    for fname in files:
        full_fname = os.path.join(file_root, fname)
        root = elemTree.parse(full_fname).getroot()
...
for sentence in formal_sentences:
    ...
    for word in sentence.split():
        cleaned_sentence.append(clean_text_MWE(word))
...
for i,y in zip(lemmatize_sentences,cleaned_sentences):
...
BNC_MWElist=[i for j in result for i in j.split() if re.search(r'_',i)!=None]
print('phrases_data:',len(BNC_MWElist))
phrases_file=open("BNC_MWElist.txt", 'w',encoding='utf-8')
for phrases in BNC_MWElist:
    phrases_file.write(phrases+'\n')
phrases_file.close()

BNC_lemma_MWElist=[i for j in lemma_sentences for i in j if re.search(r'_',i)!=None]
print('phrase_data:',len(BNC_lemma_MWElist))
phrase_file=open("BNC_lemma_MWElist.txt", 'w',encoding='utf-8')
for phrases in BNC_lemma_MWElist:
...
bnc_word=[j for i in lemma_sentences for j in i if j not in error]
bnc_word=[i for i in bnc_word if i!='']
bnc_word=[i for i in bnc_word if i not in BNC_lemma_MWElist]
bnc_word=[clean_text_word(i) for i in bnc_word]

print('bnc_n:',[i for i in bnc_word if i=='n'])
print('bnc_s:',[i for i in bnc_word if i=='s'])
print('bnc word:', len(bnc_word))

print(bnc_word[:20])


word_list_file=open('./BNC_count.txt','w',encoding='utf-8')
for i in bnc_word:
