...
os.chdir('C:/')
''' get the address of every file. This code will be used for TV corpus and COCA corpus'''
tv_data_path=[]
tv_data=os.listdir('./coca_academic')
for i in tv_data:
    tv_data_path.append(os.path.join(os.getcwd(),'coca_academic',i))


'''open the corpus and get the text'''
tv_raw_word=[]
for i in tv_data_path[:12]:
    print(i)
...
tv_raw_words=[clean_text_pre(i) for i in tv_raw_word]
tv_words=[clean_text_MWE(j) for i in tv_raw_words for j in i.split()]
...
telly_words=[i for i in tv_words if i.isalpha()==True]
print(telly_words[:15])
print('len telly:', len(telly_words))

#telly_words=[clean_text(i) for i in telly_words_word]
print('telly_words (comic_strip):',[i for i in telly_words if i=='comic_strip'])
print('error:',len([i for i in telly_words if re.search('_',i)!=None]))
print('<p> token in corpus:', len([i for i in telly_words if re.search('<p>',i)!=None]))

print('telly_words:',telly_words[:50])
...
''' lemmatize the word'''
lemma_telly_words=[]
for i in telly_words:
    lemma_telly_words.append(wn.morphy(i))
...
for i,x in enumerate(lemma_telly_words):
    if x==None:
        lemma_telly_words[i]=telly_words[i]
...
lemma_tvs=[correction(i) for i in lemma_telly_words]
...
print('raw_length:', len(telly_words))
...
for index, word in enumerate(lemma_tv):
    if '_' in word:
        ...
        telly_words[index]='_'.join(telly_words[index:index+len(word.split('_'))])
        del telly_words[index+1:index+len(word.split('_'))]

'''make MWE list'''
phrase_list=[i for i in telly_words if re.search(r'_',i)!=None]
phrase_lemmalist=[i for i in lemma_tv if re.search(r'_',i)!=None]

print(phrase_list[:10])
print(phrase_lemmalist[:10])

print('phrase list:',len(phrase_list))
print('phrase lemmalist:',len(phrase_lemmalist))

''' make word list'''
lemma_wordlist=[i for i in lemma_tv if re.search(r'_',i)==None]
lemmatized_wordlist=[clean_text_word(i) for i in lemma_wordlist]

print('lemma wordlist:',len(lemma_wordlist))

os.chdir('C:/Users/gartl/Documents')

TV_MWElist_file=open('./coca_academic_MWE1.txt','w',encoding='utf-8')
for i in phrase_list:
    TV_MWElist_file.write(i+'\n')
TV_MWElist_file.close()

lemma_TV_MWElist_file=open('./lemma_coca_academic_MWE1.txt','w',encoding='utf-8')
for i in phrase_lemmalist:
    lemma_TV_MWElist_file.write(i+'\n')
lemma_TV_MWElist_file.close()

lemma_TV_wordlist_file=open('./coca_academic_count1.txt','w',encoding='utf-8')
for i in lemmatized_wordlist:
    lemma_TV_wordlist_file.write(i+'\n')
lemma_TV_wordlist_file.close()
